...
function STORE_PREVIEW_POSITION_IN {
    ...
    case "${preview_position}" in
        left|up|top)
            ...
            ;;
        right)
            _preview_x=$((terminal_width - preview_width - 2 + (terminal_width * 6 / 100)))
            _preview_y=$((1 + fzf_offset_y + (terminal_lines * 6 / 100)))
            ;;
        down|bottom)
            _preview_x=2
...
function ADD_PLACEMENT {
    ...
    STORE_PREVIEW_POSITION_IN \
        ...
        "${preview_height}" "${preview_width}" "$terminal_lines"
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    SHELL="${BASH_BINARY}" \
        fzf --preview "DRAW_PREVIEW {}" \
            ...
            --ansi --bind "${REDRAW_KEY}:${REDRAW_COMMAND}" \
            "${@}"
fi

...
function M.run()
  ...
  local normal_submit = buffer_map(editor_buf, "n", "<CR>")
  local insert_submit = buffer_map(editor_buf, "i", "<C-s>")
  local shared_submit = buffer_map(editor_buf, "i", "<C-Space>")
  assert_equal(type(normal_submit.callback), "function", "Note editor uses the normal scratch submit key")
  assert_equal(type(insert_submit.callback), "function", "Note editor uses the insert scratch submit key")
  assert_equal(type(shared_submit.callback), "function", "Note editor keeps the shared scratch submit key")
  vim.api.nvim_buf_set_lines(editor_buf, 0, -1, false, { "Explain why this works.", "Then simplify the implementation." })
  normal_submit.callback()
  ...
  contains(long_rendered, "Explain why this works.", "multiline Note text")

...
function M.run()
  ...
  local reserved, reserved_index = Queue.take(session, third.id)
  assert_equal(third.id, reserved.id, "queue reservation")
  assert_equal(1, #Queue.list(session), "reserved prompt leaves queue")
  assert_equal(third.id, assert(Queue.restore(session, reserved, reserved_index)).id, "failed send restores prompt")
  assert_equal({ first.id, third.id }, vim.tbl_map(function(item) return item.id end, Queue.list(session)),
    "restored prompt returns to its original position")

...
local result = require("tests.e2e.harness").run({
  output = output,
  provider = provider,
  scenario = vim.env.LAZYAGENT_E2E_SCENARIO or "read-only-smoke",
  authorized = vim.env.LAZYAGENT_E2E_AUTHORIZED,
...
print(string.format("e2e %s/%s: %s", result.provider, result.scenario, result.status))

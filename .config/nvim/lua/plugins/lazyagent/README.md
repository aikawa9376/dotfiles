...
Neovim から Antigravity / Gemini / Claude / Codex / Copilot / Cursor などの agent CLI に指示を送るためのローカルプラグインです。
...
| Completion | scratch 内の `/` command、`@` path、`#notes` などの context token 補完 |
| Notes | コード範囲に AI 向けの質問・指示を保存し、scratch からまとめて送信 |
...
- 利用する agent CLI（例: `agy`, `gemini`, `claude`, `codex`, `copilot`, `cursor-agent`）
- `curl`（`edit_blocks.transport = "api"` の API edit、または画像 URL 添付を使う場合）
...
require("lazyagent").setup({
  ...
  image_paste = {
    ...
    import = {
      copy = true, -- picker で選んだ local image を conversation scope へ copy
    },
    picker = {
      recent_limit = 20,
    },
  ...
  interactive_agents = {
    Antigravity = {
      cmd = "agy",
      acp = false, -- 現行の公式 CLI は native ACP を公開していないため通常 backend を使う
      yolo = true,
    },
    Gemini = {
      cmd = "gemini",
...
添付画像は `@/absolute/path/to/image.png` の行として挿入し、`folke/snacks.nvim` のimage機能が使える場合はinline previewします。Screenshotは`WAYLAND_DISPLAY` / `DISPLAY`とOSから環境を判定し、Linuxの`import`（ImageMagick / X11）または`grim+slurp`（Wayland）、macOSの`screencapture -i`、WindowsのSnipping Tool / screen clipを利用します。Wayland上でX11用の`import`へfallbackすることはありません。ACP sessionではpickerとtranscript footerにimage input capabilityを表示し、非対応agentへ添付した場合は送信前に警告します。

scratchへ画像file pathや画像URLをdrag & dropした場合も、`image_paste.drop.enabled = true`なら自動で取り込みます。query string付き画像URLやSVGも対象です。defaultの`image_paste.dir_layout = "conversation"`では、画像を`image_paste.dir`配下のconversation / live transcript / scratchごとの`.../<scope>/images/`へ保存します。pickerで選んだlocal fileは`image_paste.import.copy`、dropしたlocal fileは`image_paste.drop.copy`を`false`にすると元pathをそのまま使います。フラット保存へ戻す場合は`image_paste.dir_layout = "flat"`を指定してください。Clipboard専用keymapは`scratch_keymaps.paste_image_normal` / `paste_image_insert` / `paste_image_insert_alt`で追加できます。
...
- 同梱の`lazyagent` skillは、live agent間通信とTeam設計を用途別referenceへ分けています。Team workflowは要望とrepository構造から必要最小限のrole・report階層・worktree方針を決め、`.lazyagent/teams.json`を生成・検証します。
...
agent scratch で `#notes` を入力すると補完と preview が表示され、送信時には追加順の番号と `@path:line` / `@path:start-end` 形式の実指示へ展開されます。ファイルや行の位置にかかわらず追加順を維持するため、複数 Note を依頼の流れとして並べられます。`:LazyAgentNotes` の一覧も同じ追加順と番号を表示します。preview だけでは消えず、送信が受理されたときに展開対象の Notes が消えます。Git status上のファイル・変更区分・インライン差分にもNoteを付けられます。`#notes`では通常のNoteと同じ`@path:line`形式を使います。行番号を解決できた場合は差分を添付せず、削除行やstaged差分など現在のファイル行を確定できない場合だけ`@path`と選択内容を送ります。対応する実ファイルにも同じNoteの印を表示します（行不明のNoteは先頭行）。折りたたみ時はファイル行、再展開時は元の選択範囲へ印を戻します。通常はworkspaceごとのメモリ上のデータです。Resessionの`lazyagent`拡張を有効にすると、セッション保存時に本文・参照・行範囲・表示設定を同じJSONの`lazyagent.notes`へ保存します。buffer番号やextmark IDは保存しません。復元はResession読込時だけ行い、セッション切替前のNotesを消して、buffer復元完了後に保存済みNotesと印を再構築します。Noteデータのない古いセッションは空として扱います。Neovim起動やファイルを開く操作だけでは保存済みNotesを読み込みません。
...
`question_policy = "prompt"` はagentからのACP `elicitation/create`を`vim.ui.select` / `vim.ui.input`で表示します。`"autonomous"`では質問を`decline`し、agent自身の既定判断で継続するよう促します。これはpermission確認には影響しません。
...
`acp.show_context_notes = true` を指定すると、`@file:range` などの添付コンテキストの直前に `Context from ...` 注釈を送信します。default では注釈だけを非表示にし、コンテキスト本体はそのまま送信します。
...
このUIはMCP serverを起動せず、active ACP sessionの一覧、prompt送信、interrupt、permission応答、最新turnのdiff reviewとfile単位Approve/Rejectを提供します。起動ごとにrandom bearer tokenを生成し、QR codeと通知URLへ含めます。未指定時は`127.0.0.1`でlocalhost限定です。LANのスマホから使う場合だけ `acp.mobile.host = "0.0.0.0"` を明示してください。LAN公開時は警告を表示し、token認証、Origin検証、request body上限を適用します。
...
footer は advertise された metadata を使って、provider / model / mode / reasoning / context usage / remaining context / turn usage / cumulative usage / provider-specific usage をできるだけ表示します。native session title / summary は `acp.show_session_summary = true` のときだけ表示します。
...
Cockpitの`t`は選択thread/worktreeのcwdでtest commandを非同期実行し、`test:running/passed/failed`をcardへ表示します。command、exit code、duration、output tailはthread metadataへ保存されます。
...
`:LazyAgentACPProtocolLog`はsession単位のprotocol flight recorderを開きます。JSONLは0600でcacheへ保存され、MCP headers、env、token/key/password系fieldを記録前にredactします。`:LazyAgentACPCapabilities`と`:LazyAgentACPDoctor`でcapability matrixとsession healthも確認できます。
...
| `:[range]LazyAgentNote [text]` | 現在行 / 選択範囲に AI 向け Note を保存。text 省略時は長文用 scratch float を表示 |

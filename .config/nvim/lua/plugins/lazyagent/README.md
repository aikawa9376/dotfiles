...
return {
  ...
  cmd = {
    ...
    "LazyAgentEdit", "LazyAgentNote", "LazyAgentNotes", "LazyAgentHistory", "LazyAgentConversationList", "LazyAgentSummary",
...
コード上に AI への質問や指示を一時保存し、scratch からまとめて送れます。normal mode では現在行、visual range では選択行に Note を付けます。
...
Note は extmark なので、保存後に前方の行を編集しても対象範囲に追従します。scratch で `#notes` を入力すると補完と preview が表示され、送信時には `@path:line` / `@path:start-end` 形式の実指示へ展開されます。preview だけでは消えず、送信が受理されたときに展開対象の Notes が消えます。Notes は workspace ごとの Neovim session 内データで、ファイルには書き込みません。
...
| `:[range]LazyAgentNote [text]` | 現在行 / 選択範囲に AI 向け Note を保存。text 省略時は入力 UI を表示 |
| `:LazyAgentNotes` | 現在 workspace の Notes 一覧を開く（`<CR>` open、`d` delete、`C` clear） |

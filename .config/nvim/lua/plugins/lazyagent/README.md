...
`:LazyAgentACPRegistry`は公式ACP Registry v1を取得し、agentを検索・登録・updateできます。npx/uvx distributionはversion-pinned launcherとしてdata directoryへ記録され、現在のNeovim設定にもACP agentとして追加されます。binary distributionは検証済みarchive installerが必要です。

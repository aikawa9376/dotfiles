...
local function release_closing_session_memory(session)
  ...
  session.subagents = {}
  session.async_tasks = {}
  session.current_plan_artifact = nil
  session.session_failures = {}
  session.session_failure_order = {}
  session.active_session_failure = nil
  session.last_failed_prompt = nil
  session.provider_compactions = {}
  session.agent_info = {}
...
sync_runtime_session = function(session)
  ...
  runtime.acp_goal = vim.deepcopy(session.goal)
  runtime.acp_subagents = vim.deepcopy(session.subagents or {})
  runtime.acp_async_tasks = vim.deepcopy(session.async_tasks or {})
  runtime.acp_plan = vim.deepcopy(session.current_plan or {})
  runtime.acp_plan_artifact = vim.deepcopy(session.current_plan_artifact)
  runtime.acp_session_failures = vim.deepcopy(session.session_failures or {})
  runtime.acp_active_session_failure = vim.deepcopy(session.active_session_failure)
  runtime.acp_provider_compactions = vim.deepcopy(session.provider_compactions or {})

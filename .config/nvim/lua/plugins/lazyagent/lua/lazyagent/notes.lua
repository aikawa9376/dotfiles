...
function M.add(opts)
  ...
  if vim.fn.strdisplaywidth(preview) > 60 then preview = vim.fn.strcharpart(preview, 0, 57) .. "…" end
  local mark_id = vim.api.nvim_buf_set_extmark(bufnr, namespace, start_line - 1, 0, {
    ...
    virt_text = { { "  " .. preview, "LazyAgentNoteText" } },
    virt_text_pos = "eol",
  })
  local entry = {
    id = next_id,
...
local function refresh_list(bufnr, root)
  ...
  local lines = { "# LazyAgent Notes", "", "<CR> open  d delete  C clear  q close", "" }
  local line_map = {}
  for _, entry in ipairs(matching(root)) do
    local row = #lines + 1
    lines[#lines + 1] = string.format("%s  %s", ref_for(entry):sub(2), entry.text:gsub("\n", " "))

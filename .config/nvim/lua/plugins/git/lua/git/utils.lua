...
function M.get_work_tree(opts)
  ...
  local cwd = name ~= '' and not name:match('^%w[%w-]*://') and (vim.fn.isdirectory(name) == 1 and name or vim.fs.dirname(name)) or vim.fn.getcwd()
  local result = vim.system({ 'git', '-C', cwd, 'rev-parse', '--show-toplevel' }, { text = true }):wait()
  if result.code == 0 then return M.normalize_path(vim.trim(result.stdout)) end
  if opts.notify then vim.notify(opts.not_git_message or 'Not in a Git repository', vim.log.levels.ERROR) end
  return nil
end
...
function M.get_filepath_at_cursor(bufnr)
  if vim.b[bufnr].custom_git_commit then
    local entry = require('git.features.commit').entry_at(bufnr, vim.api.nvim_win_get_cursor(0)[1])

...
function M.show_commit_info_float(commit, toggle, create_if_missing)
  ...
  local commit_info = require('git.features.commit_info').lines(work_tree, commit)
...
function M.setup()
  vim.api.nvim_create_user_command('GeditHeadAtFile', function()
    local root, path = require('git.objects').context()
    local hash = require('git.objects').run(root, { 'log', '-1', '--format=%H', '--', path })
    if vim.trim(hash) == '' then error('No commits for ' .. path) end
    local view = require('git.features.commit')
    local buf = view.open({ work_tree = root, revision = vim.trim(hash), tab = true })
    if not buf then return end
    view.expand_file(buf, path)
    for row = 1, vim.api.nvim_buf_line_count(buf) do
      local entry = view.entry_at(buf, row)
      if entry and entry.path == path then vim.api.nvim_win_set_cursor(0, { row, 0 }); break end
    end
  end, {})

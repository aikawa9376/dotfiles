...
local function dim_info()
  for _, win in ipairs(vim.api.nvim_list_wins()) do
    ...
    if title and title[1] and title[1][1] == ' Dimmed Commit ' then
      return win, text(vim.api.nvim_win_get_buf(win))
    end
  end
end
vim.cmd('DiffDim')
...
assert(#vim.api.nvim_buf_get_extmarks(original, dim_ns, 0, -1, {}) == 0, 'closing blame must clear its code dim')
assert(vim.api.nvim_win_get_buf(code_win) == original and vim.bo[original].modified)

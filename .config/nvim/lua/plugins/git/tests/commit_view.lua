...
local line_change
vim.api.nvim_buf_attach(b, false, { on_lines = function(_, _, _, first, old_end, new_end)
  line_change = { first = first, old_end = old_end, new_end = new_end }
end })
local closed_count = vim.api.nvim_buf_line_count(b)
focus('M a file.txt'); press('o')
assert(line_change and line_change.first > 0 and line_change.old_end < closed_count,
...
press('i')
assert(vim.api.nvim_win_get_cursor(0)[1] == find('@@'), 'i should move to the next hunk')
assert(vim.api.nvim_buf_get_changedtick(b) == expanded_tick, 'moving within an open diff must not redraw it')
press(']m')
assert(vim.api.nvim_win_get_cursor(0)[1] == find('A deleted.txt'), 'file navigation should reach the next entry')
local moved_tick = vim.api.nvim_buf_get_changedtick(b)
press('[m')
assert(vim.api.nvim_win_get_cursor(0)[1] == find('M a file.txt'), 'reverse file navigation should reach the prior entry')
assert(vim.api.nvim_buf_get_changedtick(b) == moved_tick, 'navigation between closed files must not redraw')
press('o')
assert(require('git.utils').get_commit(b) == second)
local subject = #api.model(b).header + 1
vim.api.nvim_buf_set_lines(b, subject - 1, subject, false, { 'edited second', 'extra paragraph' })
local draft_tick = vim.api.nvim_buf_get_changedtick(b)
focus('M a file.txt'); press('i')
assert(vim.api.nvim_win_get_cursor(0)[1] == find('@@'), 'i should account for edited message lines')
assert(vim.api.nvim_buf_get_changedtick(b) == draft_tick, 'hunk navigation must preserve the draft without redrawing')
focus('M a file.txt'); press('o')

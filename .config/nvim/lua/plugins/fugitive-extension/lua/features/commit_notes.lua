...
local M = {}
local function api() return require('features.commit') end
function M.capture(buf, first, last)
  ...
  local patch = require('features.commit_model').patch(model, entry)
  local hunk
  ...
  local blob = require('features.commit_model').git(model.root, { 'rev-parse', '--verify', source.revision .. ':' .. source.path })

...
local model = require('features.blame_model')
local sessions = {}
local pending_opens = {}
local serial = 0
...
local function history_info(s, refresh)
  ...
  if refresh or s.info_revision ~= f.revision then
    ...
    s.info_lines = require('features.commit_info').lines(s.root, f.revision) or { 'Could not load commit information' }
  end
...
local function open_commit(s, layout)
  ...
  local b = require('features.commit').open({ work_tree = s.root, revision = hash, split = layout == 'split', tab = layout ~= 'split' })
  if b then
    ...
    local commit = require('features.commit')
...
bind = function(s, b, code)
  ...
  map({ 'g?', '<F1>' }, function() require('features.help').show('Git blame', {
    '- / s / u  blame at the selected commit', '~ / <BS>  blame before the change (count supported)',
    '{count}P   blame a numbered parent', 'Ctrl-o / Ctrl-i  back / forward (code and blame together)',
    'gk         toggle following commit message', 'Ctrl-p / p toggle / open following diff preview',
    '<CR> / i   inspect commit in a tab (q returns)', 'o / O      open commit in split / tab',
    'd          compare before/after the change', 'c          absolute / relative date heatmap',
    '( / )      previous / next commit block', 'y          copy full hash', '.          put hash on command line',
    'A / C / D  full / hash / date panel width', 'R          refresh working-tree blame', 'q / gq     close and restore original file',
  }) end)
  map('d', function()
...
function M.setup(group)
  ...
  require('features.blame_legacy').setup(group)
  vim.api.nvim_create_user_command('GitBlameLegacy', function(cmd)
    vim.cmd('Git blame -w --date=format:%Y-%m-%d\\ %H:%M ' .. cmd.args)
  end, { nargs = '*', desc = 'Open legacy Fugitive blame with optional flags' })

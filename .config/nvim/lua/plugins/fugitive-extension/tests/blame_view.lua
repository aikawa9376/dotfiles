...
local api = require('features.blame')
...
require('features.commit').setup(vim.api.nvim_create_augroup('BlameCommitTest', { clear = true }))
b = api.open()
...
press('q')
-- Native blame and its old extensions still work when explicitly requested.
vim.opt.rtp:prepend(vim.fn.stdpath('data') .. '/lazy/vim-fugitive')
vim.cmd('runtime plugin/fugitive.vim')
vim.cmd('edit ' .. vim.fn.fnameescape(root .. '/new name.txt'))
vim.cmd('GitBlameLegacy')
assert(vim.bo.filetype == 'fugitiveblame')
assert(vim.fn.maparg('c', 'n', false, true).callback)
assert(vim.fn.maparg('d', 'n', false, true).callback)
assert(vim.fn.maparg('<C-p>', 'n', false, true).callback)

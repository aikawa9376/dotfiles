...
local original = vim.api.nvim_buf_get_lines(b, 0, -1, false)
local name = vim.api.nvim_buf_get_name(b)
press('<CR>')
local blob = vim.api.nvim_get_current_buf()
assert(vim.api.nvim_buf_get_name(blob):match('^git%-commit%-blob://'), 'blob should use the custom view')
...
assert(vim.deep_equal(vim.api.nvim_buf_get_lines(b, 0, -1, false), original), 'expanded/collapsed file state was lost')

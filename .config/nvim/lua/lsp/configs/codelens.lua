...
function M.run()
  ...
  elseif #options == 1 then
    ...
    open_action_float(options, bufnr, options)
  end
end

function open_action_float(result, bufnr, options)
  ...
  local fmt =  '<cmd>lua execute_float_lens(%d, %d)<CR>'
  vim.api.nvim_buf_set_var(buf, 'code_action_result', result)
  vim.api.nvim_buf_set_var(buf, 'code_action_bufnr', bufnr)
  vim.api.nvim_buf_set_var(buf, 'code_action_options', options)
...
function get_titles_length(result)
  local option_strings = {"Code lenses:"}
  local length = 0
  for i, action in ipairs(result) do
    local title = string.format('%d. %s', i, action.lens.command.title)
...
---@private
function execute_float_lens(winnr, float_buf)
  local bufnr = vim.api.nvim_buf_get_var(float_buf, 'code_action_bufnr')
  local result = vim.api.nvim_buf_get_var(float_buf, 'code_action_result')
  local options = vim.api.nvim_buf_get_var(float_buf, 'code_action_options')
  ...
  local client_id = option.client
  local lens = option.lens

  execute_lens(lens, bufnr, client_id)
  vim.api.nvim_win_close(winnr, true)
end

function execute_lens(lens, bufnr, client_id)
